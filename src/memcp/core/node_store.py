...
class NodeStore:
    ...
    def _init_conn(self) -> sqlite3.Connection:
        ...
        # In-process wait for a contended DB matches the cross-process write
        # lock's wait (default 30s). 5s was not enough on a cold first
        # session: the initial embedding-model fetch/load mid-write-storm
        # GIL-starves a thread holding an open transaction past the timeout,
        # and concurrent remembers die with 'database is locked'.
        conn.execute(f"PRAGMA busy_timeout={config.write_lock_timeout * 1000}")
