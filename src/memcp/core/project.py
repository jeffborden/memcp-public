...
#
# state.json's current_project/current_session are machine-global: every
# server startup overwrites them, so any process that reads them at write
# time stamps its writes with whichever server registered LAST (observed:
# two Claude sessions both attributed to one session id). A server therefore
# resolves its identity ONCE at startup and keeps it in process memory;
# state.json remains as a last-registered record for external readers, but
# a process that has an identity never consults it again.

_process_project: str | None = None
_process_session: str | None = None


def set_process_identity(project: str, session_id: str) -> None:
    """Pin this process's project/session. Called once at server startup."""
    global _process_project, _process_session
    _process_project = project
    _process_session = session_id


def clear_process_identity() -> None:
    """Unpin process identity (test isolation)."""
    global _process_project, _process_session
    _process_project = None
...
def get_current_session() -> str:
    """Get this process's session ID.

    Prefers the process-local identity pinned at server startup; falls back
    to state.json only for callers outside a running server (scripts, tests).
    Returns '' if unset.
    """
...
def set_current_session(session_id: str) -> None:
    """Set current session in state.json and, if pinned, process memory."""
...
def get_current_project() -> str:
    """Get this process's project.

    Prefers the process-local identity pinned at server startup; falls back
    to state.json, then auto-detection, for callers outside a running server.
    Returns the project name (never empty — falls back to "default").
    """
    if _process_project is not None:
