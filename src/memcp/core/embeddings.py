...
# Consecutive tier-load failures. A transient failure leaves _provider_loaded
# False so a later call retries the genuine tier (no-churn discipline), but a
# persistently failing model (e.g. a corrupted model file) must not re-attempt
# the load on every search — after _MAX_LOAD_ATTEMPTS the keyword-only degrade
# is cached until reset_provider() / process restart.
...
def get_provider() -> EmbeddingProvider | None:
    ...
    """
    global _cached_provider, _provider_loaded, _load_attempts
    ...
    try:
        ...
        # flag clear so a later call retries the genuine tier — but only up to
        # _MAX_LOAD_ATTEMPTS: a persistently failing model (corrupted file)
        # must not re-run the load on every search.
        _load_attempts += 1
        if _load_attempts >= _MAX_LOAD_ATTEMPTS:
            _provider_loaded = True
            logger.warning(
                "embedder tier %r failed to load %d consecutive times; caching "
                "keyword-only degrade until reset_provider()/restart (no "
                "model_version flip)",
                tier,
                _load_attempts,
            )
