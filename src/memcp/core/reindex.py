...
def rebuild_embeddings(mode: str = "incremental", force: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        if mode == "incremental":
            ...
            prev_seq = meta["built_against_seq"] if meta else -1
            # Single pass: fetch id + content + ingest_seq for all ACTIVE nodes
            # (archived filtered, P4(d) parity with the edges/entities builders),
            # partition into "new" (ingest_seq past the cut → to embed) vs
            # "live_ids" (to check orphan status against). A previously-embedded
            # node that gets archived drops out of live_ids and its vector is
            # swept as an orphan. The cut is ingest_seq, not created_at, so
            # merged rows (older created_at, fresh seq) and clock-skewed writes
            # are never missed (§3.4).
            all_rows = conn.execute(
                "SELECT id, content, ingest_seq FROM nodes WHERE archived_at IS NULL"
            ).fetchall()
        ...
        # Meta-write last, in one transaction (P4(a) parity with the
        # edges/entities builders).
        with conn.atomic():
            set_index_meta(
                conn,
