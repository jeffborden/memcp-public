...
def recall(
    ...
    # Time the recall for the latency gauge (feeds the codified ANN-index
    # trigger via memcp_status). Emit only on success — a raised recall isn't a
    # latency data point. ``mode`` labels the retrieval path so an operator can
    # see whether the vector-sweep (semantic) or the ranker floor (keyword)
    # dominates; a failed semantic provider still counts as an attempted
    # semantic path (its degrade is logged separately in semantic_recall).
    graph_backend = _use_graph()
    start = time.perf_counter()
    if graph_backend:
        results = _recall_graph(
            query=query,
            category=category,
            importance=importance,
            limit=limit,
    ...
    if not query.strip():
        mode = "filter"
    elif graph_backend and use_graph and get_config().semantic_recall_enabled:
        mode = "semantic"
    else:
        mode = "keyword"
    telemetry.emit_recall(mode=mode, dur_ms=dur_ms, results=len(results))
...
def _recall_json(
    ...
    # Apply token budget (RecallResults carries .truncation when items were cut)
    if max_tokens > 0:
        from memcp.core.recall_meta import apply_token_budget

        insights = apply_token_budget(insights, max_tokens)
...
def memory_status(project: str = "", session: str = "") -> dict[str, Any]:
    ...
    Scoped counts (total_insights etc.) are labeled by ``scope`` and the
    whole-store picture always rides along in ``store`` — a project-scoped
    "total_insights: 196" with no store total hid a 196-vs-393 split for two
    weeks; the response must say what its numbers cover.
    """
    ...
    if _use_graph():
        result = _status_graph(project=project, session=session)
    else:
        result = _status_json(project=project, session=session)
    ...
    result["scope"] = scope
    result["store"] = _store_totals()
    return result


def _store_totals() -> dict[str, Any]:
    """Whole-store insight counts, broken down by project (active only)."""
    ...
    if _use_graph():
        ...
        try:
            ...
            rows = conn.execute(
                ...
                "WHERE archived_at IS NULL GROUP BY project"
            ).fetchall()
            for row in rows:
                by_project[row["project"] or "default"] = row["cnt"]
        finally:
            graph.close()
    else:
        memory = _load_memory()
        for ins in memory["insights"]:
            name = ins.get("project") or "default"
            by_project[name] = by_project.get(name, 0) + 1

    return {
        "total_insights": sum(by_project.values()),
...
def _status_graph(project: str = "", session: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        # plus the recall-latency gauge that backs the codified ANN-index trigger.
        result["telemetry"] = {
            "enabled": config.telemetry_enabled,
            "dir": config.telemetry_dir,
            "recall_latency": telemetry.recall_latency_summary(),
        }
