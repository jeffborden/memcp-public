...
def _emit_remember_fields(
    *, tags: str, summary: str, importance: str, category: str, result: dict[str, Any]
) -> None:
    """Telemetry: received-vs-stored field shapes for every remember.

    Issue #82 reported saves whose tags/summary/importance vanished; it has
    never reproduced server-side (the full-MCP-stack integrity test is green),
    so the next occurrence needs evidence of what the server actually
    RECEIVED. Metadata only — counts, bools and bounded enums, never tag
    values or content. Fail-open like all telemetry.
    """
    try:
        received = {
            "tags": len([t for t in tags.split(",") if t.strip()]) if tags else 0,
            "summary": bool(summary.strip()),
            "importance": importance,
            "category": category,
        }
        stored = {
            "tags": len(result.get("tags", [])),
            "summary": bool(result.get("summary", "").strip()),
            "importance": result.get("importance", ""),
            "category": result.get("category", ""),
            "status": "duplicate" if result.get("_duplicate") else "saved",
        }
        telemetry.emit("remember_fields", "memcp_remember", received=received, stored=stored)
...
async def memcp_remember(
    ...
    Project and session are stamped from the server's own identity and are
    deliberately NOT parameters: when they were, models filled them in ~78%
    of calls and forked half the store out of the default read path.
    ...
    try:
        ...
        _emit_remember_fields(
            tags=tags, summary=summary, importance=importance, category=category, result=result
        )
...
async def memcp_recall(
    ...
    Default scope is "all": the data dir is the isolation boundary, and a
    scope="project" default hid ~half a store whose writes had forked across
    project labels (docs/design/default-scope.md). Narrow explicitly when
    you want one project's view.
    ...
    Args:
        ...
        max_tokens: Token budget — returns results until budget is exhausted.
            Default 8000 to stay under the typical context cap; pass 0 for
            unlimited. A budget cut is reported in the "truncated" field.
        project: Filter by project
        session: Filter by session ID
        scope: "all" (default, whole store), "project", "session" (current only)
    ...
    try:
        ...
        # RecallResults carries .truncation when the token budget cut results
        # — surface it, or "2 results" reads as "only 2 matches exist".
        truncation = getattr(results, "truncation", None)
        if truncation:
            payload["truncated"] = truncation
            payload["message"] = (
                f"Token budget max_tokens={truncation['budget']} truncated results: "
                f"{truncation['returned']} of {truncation['matched']} matched insights "
                f"returned. Raise max_tokens or pass 0 for unlimited."
            )
...
def memcp_forget(insight_id: str) -> str:
    ...
    Requires the FULL id — no prefix resolution, by design: forget is
    destructive and unrecoverable, so resolve a prefix via memcp_get first.

    Args:
        insight_id: The full ID of the insight to remove
    """
...
def _resolve_insight_id(insight_id: str) -> tuple[str, str | None]:
    """Resolve a full id or an unambiguous prefix to a full id (memcp_get's
    contract) for the recoverable mutation tools (update/archive).

    Returns ``(id_to_use, None)`` normally, or ``(insight_id, ambiguous_json)``
    when a prefix matches 2+ active insights. An id that resolves to nothing is
    passed through unchanged so the underlying operation reports not_found
    exactly as before (and exact ids of archived rows keep their pre-existing
    behavior).
    """
    resolved = get_insight(insight_id)
    if isinstance(resolved, list):
        return insight_id, json.dumps(
            {
                "status": "ambiguous",
                "prefix": insight_id,
                "count": len(resolved),
                "candidates": [r.get("id") for r in resolved],
            },
            indent=2,
            default=str,
        )
    if resolved is not None:
        return str(resolved["id"]), None
    return insight_id, None
...
def memcp_update(
    ...
    Args:
        insight_id: ID of the insight to update — full id or an unambiguous
            prefix (same resolution as memcp_get; ambiguous prefixes return
            status "ambiguous" with the candidate ids).
    ...
    """
    try:
        insight_id, ambiguous = _resolve_insight_id(insight_id)
...
def memcp_archive(insight_id: str) -> str:
    ...
    Args:
        insight_id: ID of the insight to archive — full id or an unambiguous
            prefix (same resolution as memcp_get).
    """
...
def memcp_load_context(
    ...
    The context's project is stamped from the server's own identity and is
    deliberately not a parameter (see memcp_remember).
    ...
    """
    return load_context(name=name, content=content, file_path=file_path)
...
async def memcp_search(
    ...
    Default scope is "all" — see docs/design/default-scope.md.
    ...
    Args:
        ...
        project: Filter by project
        scope: "all" (default, whole store), "project", "session"
    """
...
async def memcp_consolidation_preview(
    ...
    """Preview similar insights split into mergeable vs unjustified.

    Similarity only NOMINATES: single-linkage grouping is transitive, so a
    raw similarity group can chain unrelated insights together (observed:
    150 of 196 insights in one group). Each nominated group is split by the
    supersession gate — "mergeable" subsets (members explicitly claim to
    supersede one another, suggested keeper included) and "unjustified"
    members (similar prose, no claim — do not merge).
    Dry-run — no changes made. Use memcp_consolidate on a mergeable subset.
...
async def memcp_consolidate(
    ...
    """Merge a group of similar insights into one. DESTRUCTIVE.

    Keeps the best insight (most accessed by default), merges tags/entities
    from others, redirects edges, and deletes duplicates. Refuses groups
    larger than 8 — oversized groups are usually transitive-similarity
    artifacts, not duplicates. Prefer the "mergeable" subsets and
    suggested_keep_id from memcp_consolidation_preview.
...
async def memcp_log_episode(
    ...
    Project and session are stamped from the server's own identity and are
    deliberately not parameters (see memcp_remember).

    Args:
        task: What was the task or goal (e.g., "Debug the flaky deploy pipeline")
        approach: How was it approached (e.g., "Bisected CI runs, compared env diffs")
...
async def memcp_recall_episodes(
    ...
    Args:
        query: Search for similar tasks (e.g., "pipeline debugging")
...
def _init_session() -> str:
    ...
    Resolves this process's identity ONCE and pins it in process memory —
    write paths must never re-read the machine-global state.json, or a
    later-starting server steals attribution of every subsequent write.
    state.json/sessions.json are still updated as the last-registered record.
    """
    ...
    set_process_identity(project, session_id)
    return session_id
