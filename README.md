...
- **Insight lifecycle.** `memcp_index` (a progressive-disclosure map of all insights), `memcp_get` (fetch by id), `memcp_update` (retag / reclassify in place, preserving edges), `memcp_archive`, and revision tracking. `memcp_get`, `memcp_update`, and `memcp_archive` all accept an unambiguous id prefix (ambiguous prefixes return the candidates); `memcp_forget` requires the full id by design.
