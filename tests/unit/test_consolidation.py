...
def test_consolidate_bumps_revision(isolated_data_dir):
    ...
    # Two near-duplicates where the newer EXPLICITLY supersedes the older —
    # bulk consolidate only merges gate-justified subsets now, so the claim
    # is what makes this pair mergeable (similarity alone only nominates).
    ...
    memory.remember(
        f"Python uses the GIL to limit the interpreter (supersedes {first['id'][:8]})",
        category="fact",
        importance="low",
    )
    ...
    result = consolidation.consolidate(threshold=0.5)
