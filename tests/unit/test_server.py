...
class TestRememberRecallForget:
    ...
    async def test_update_by_prefix(self, isolated_data_dir: Path) -> None:
        created = json.loads(
            await memcp_remember(content="Prefix update target", category="general")
        )
        insight_id = created["id"]
        result = json.loads(memcp_update(insight_id=insight_id[:8], tags="kind:kb,prefixed"))
        assert result["status"] == "updated"
        assert result["insight"]["id"] == insight_id

    async def test_update_ambiguous_prefix_returns_candidates(
        self, isolated_data_dir: Path
    ) -> None:
        # 17 inserts over 16 hex first-chars pigeonholes a shared 1-char prefix.
        ids = []
        for i in range(17):
            created = json.loads(
                await memcp_remember(content=f"Ambiguity corpus row {i}", category="general")
            )
            ids.append(created["id"])
        first = next(c for c in {i[0] for i in ids} if sum(1 for x in ids if x[0] == c) >= 2)
        result = json.loads(memcp_update(insight_id=first, tags="kind:kb,should-not-apply"))
        assert result["status"] == "ambiguous"
        assert result["count"] >= 2
        assert all(c.startswith(first) for c in result["candidates"])

    async def test_archive_by_prefix(self, isolated_data_dir: Path) -> None:
        created = json.loads(
            await memcp_remember(content="Prefix archive target", category="general")
        )
        insight_id = created["id"]
        result = json.loads(memcp_archive(insight_id[:8]))
...
class TestProjectSessionTools:
    ...
    async def test_projects_after_remember(self, isolated_data_dir: Path) -> None:
        # The tool no longer takes project= (attribution comes from process
        # identity) — pin the identity the way server startup does.
        from memcp.core.project import set_process_identity

        set_process_identity("test-proj", "test-proj_2026-01-01_001")
        await memcp_remember(content="Project listing test", category="fact")
        result = json.loads(memcp_projects())
        assert result["status"] == "ok"
        assert result["count"] >= 1
