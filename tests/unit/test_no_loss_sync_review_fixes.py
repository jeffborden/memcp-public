...
def test_model_version_change_forces_full_rebuild(monkeypatch) -> None:
    from memcp.core import memory
    from memcp.core import reindex as reindex_mod
    ...
    # Pin the embedding provenance: without this, on a machine lacking the
    # semantic extras the CURRENT edges model_version carries the
    # keyword-fallback token, and the P4(e) fallback exception (correctly)
    # suppresses the full-rebuild promotion — making the test pass or fail
    # based on which optional deps are installed rather than on the behavior
    # under test.
    monkeypatch.setattr(
        reindex_mod, "_current_embedding_model_version", lambda: "pinned-embedder-v1"
    )
