...
@pytest.mark.skipif(not NUMPY_AVAILABLE, reason="numpy not installed")
def test_rebuild_embeddings_full_excludes_archived(
    seeded_graph: GraphMemory, monkeypatch: pytest.MonkeyPatch
) -> None:
    """P4(d) parity with edges/entities: archived nodes are not embedded."""
    import memcp.core.embeddings as emb
    ...
    victim_id = conn.execute("SELECT id FROM nodes LIMIT 1").fetchone()[0]
    conn.execute(
        "UPDATE nodes SET archived_at = '2026-01-01T00:00:00+00:00' WHERE id = ?",
        (victim_id,),
    )
    conn.commit()

    result = rebuild_embeddings(mode="full", force=True)
    assert result["items"] == 2, "only the two active nodes are embedded"
...
@pytest.mark.skipif(not NUMPY_AVAILABLE, reason="numpy not installed")
def test_rebuild_embeddings_incremental_sweeps_archived(
    seeded_graph: GraphMemory, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A previously-embedded node that gets archived is swept as an orphan on
    the next incremental rebuild (its vector must not keep serving recall)."""
    import memcp.core.embeddings as emb
