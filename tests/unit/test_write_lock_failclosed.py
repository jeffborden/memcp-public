...
def test_write_lock_error_is_memcp_error() -> None:
    """Lock failures must surface as MemCPError so the tool layer returns a
    clean client error instead of an opaque RuntimeError traceback."""
    from memcp.core.errors import MemCPError

    assert issubclass(wl.WriteLockError, MemCPError)
