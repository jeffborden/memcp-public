...
16. Per-pull convergence audit: the audit runs inside the pull cycle (not only
    on demand) whenever state could have moved, and skips no-change ticks.
"""
...
# ── Test 16 — the convergence audit runs per pull cycle, skipping no-ops ──────


def test_convergence_audit_runs_every_pull_cycle(tmp_path: Path) -> None:
    """The audit runs inside the pull cycle (not only on demand) on every tick
    where state could have moved — a new published snapshot or pending local
    durable writes — and skips the re-audit on no-change ticks (the punch-list
    quieting: re-auditing an unchanged world was pure noise)."""
    drive = tmp_path / "drive"
    sA = _machine(tmp_path, "A", [("a1", "A1")], drive)
    _publish(sA)
    ...
    sB._flush_tick()
    assert calls["n"] == 1, "the convergence audit must run in the pull cycle, not on demand"
    assert sB._last_audit, "the audit result must be recorded"

    # Drain one tick (the first pull may repush B's superset, moving the
    # pointer), then verify no-change ticks skip the re-audit.
    sB._flush_tick()
    settled = calls["n"]
    sB._flush_tick()
    sB._flush_tick()
    assert calls["n"] == settled, "no-change ticks must not re-audit"
    assert sB._audit_count == settled

    # A fresh peer publish (new pointer written_at) re-arms the audit.
    time.sleep(0.01)
    _add_node(sA.local_db, "a2", "A2")
    sA.mark_durable_dirty()
    assert sA.push(force=True) is True
    sB._flush_tick()
    assert calls["n"] == settled + 1, "a new publish must trigger a re-audit"
