...
class TestLoadRetryCap:
    """A persistently failing tier (e.g. a corrupted model file) must stop
    re-attempting the load after _MAX_LOAD_ATTEMPTS instead of re-running it
    on every search; reset_provider() re-opens the retries.
    """

    def _force_hq_failing(self, monkeypatch: pytest.MonkeyPatch, calls: dict) -> None:
        reset_provider()
        monkeypatch.delenv("MEMCP_EMBEDDING_PROVIDER", raising=False)
        monkeypatch.delenv("MEMCP_EMBEDDER_TIER", raising=False)
        monkeypatch.setattr(emb, "_fastembed_available", lambda: True)
        monkeypatch.setattr(emb, "_model2vec_available", lambda: False)

        def _corrupt(*_a, **_k):
            calls["n"] += 1
            raise RuntimeError("corrupted model file")

        monkeypatch.setattr(emb, "FastEmbedProvider", _corrupt)

    def test_load_attempts_capped(self, monkeypatch) -> None:
        calls = {"n": 0}
        self._force_hq_failing(monkeypatch, calls)

        for _ in range(emb._MAX_LOAD_ATTEMPTS + 2):
            assert get_provider() is None
        assert calls["n"] == emb._MAX_LOAD_ATTEMPTS, "load must not be re-attempted past the cap"
        reset_provider()

    def test_reset_provider_reopens_retries(self, monkeypatch) -> None:
        calls = {"n": 0}
        self._force_hq_failing(monkeypatch, calls)

        for _ in range(emb._MAX_LOAD_ATTEMPTS + 1):
            get_provider()
        assert calls["n"] == emb._MAX_LOAD_ATTEMPTS

        reset_provider()
        get_provider()
        assert calls["n"] == emb._MAX_LOAD_ATTEMPTS + 1, (
            "reset_provider must re-open the retry budget"
        )
        reset_provider()
