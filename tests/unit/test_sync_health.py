...
# ── (d) per-tick audit quieting ────────────────────────────────────────


def test_tick_audit_warns_on_real_divergence(
    tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """A nonzero row delta OUTSIDE the write-to-push window still warns —
    the quieting must not hide a genuine silent no-merge."""
    db_a = tmp_path / "a" / "graph.db"
    db_b = tmp_path / "b" / "graph.db"
    drive = tmp_path / "drive"
    _make_db(db_a, [("n1", "row"), ("n2", "row")])
    _make_db(db_b, [("n1", "row")])
    _publish(db_a, drive)

    s = _sync(db_b, drive)
    s.pull_if_newer = lambda *a, **k: False  # type: ignore[method-assign]  # simulate silent no-merge
    with caplog.at_level(logging.DEBUG):
        s._flush_tick()

    assert any(
        "diverged" in r.getMessage() and r.levelno == logging.WARNING for r in caplog.records
    ), "genuine divergence (delta != 0, no pending local writes) must warn"


def test_tick_audit_debug_in_write_to_push_window(
    tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """Divergence while durable local writes await the push is expected churn
    and logs at debug, not warning."""
    db_a = tmp_path / "a" / "graph.db"
    drive = tmp_path / "drive"
    _make_db(db_a, [("n1", "row")])
    _publish(db_a, drive)

    s = _sync(db_a, drive)
    # New local row awaiting push — the audit inside the tick runs while
    # _durable_dirty is still set (push happens after the audit).
    conn = sqlite3.connect(str(db_a))
    conn.execute(
        "INSERT INTO nodes (id, content, created_at) VALUES (?, ?, ?)",
        ("n2", "row", "2026-01-01T00:00:00+00:00"),
    )
    conn.commit()
    conn.close()
    s.mark_durable_dirty()
    s.push = lambda *a, **k: True  # type: ignore[method-assign]  # hold the window open

    with caplog.at_level(logging.DEBUG):
        s._flush_tick()

    diverged = [r for r in caplog.records if "diverged" in r.getMessage()]
    assert diverged, "the audit must still run and report inside the dirty window"
    assert all(r.levelno == logging.DEBUG for r in diverged), (
        "write-to-push-window divergence must log at debug, not warning"
    )


def test_tick_audit_skipped_when_nothing_changed(tmp_path: Path) -> None:
    """No new publish and no pending local writes → the re-audit is skipped;
    ...
    s = _sync(db_a, drive)
    s._flush_tick()
    first = s._audit_count
    assert first == 1

    s._flush_tick()
    s._flush_tick()
    assert s._audit_count == first, "unchanged pointer + clean local → no re-audit"

    # A peer publish (new pointer written_at) re-arms the audit. Pushing
    # identical content is deduped, so give the publish a real new row.
    time.sleep(0.01)
    _make_db(db_a, [("n1", "row"), ("n2", "row")])
    _publish(db_a, drive)
    s._flush_tick()
    assert s._audit_count == first + 1, "a new publish must trigger a re-audit"
